...
namespace myNS
{
    ...
    public:
        ...
        std::list<myNS::Point*> GetPath(Point &startPoint, Point &endPoint, bool isIgnoreCorner,
                                        std::list<Point *> &_openlist, std::list<Point *> &_closelist);
    private:
    ...
    Point *Astar::findPath(Point &startPoint,Point &endPoint,bool isIgnoreCorner)
    {
        ...
        while(!openList.empty())
        {
            ...
            if (prePoint == NULL)
                ...
                direction=Point(curPoint->x-prePoint->x,curPoint->y-prePoint->y);
      ...
      /*  for(int x=point->x-1;x<=point->x+1;x++)
        ...
        else if( direction.x!=0&&direction.y!=0)
        {
            if(isCanreach(point,new Point(point->x+direction.x,point->y+direction.y),isIgnoreCorner))
                surroundPoints.push_back(new Point(point->x+direction.x,point->y+direction.y));
            if(isCanreach(point,new Point(point->x,point->y+direction.y),isIgnoreCorner))
                surroundPoints.push_back(new Point(point->x,point->y+direction.y));
            if(isCanreach(point,new Point(point->x+direction.x,point->y),isIgnoreCorner))
                surroundPoints.push_back(new Point(point->x+direction.x,point->y));
        }
       else
        {
            if(isCanreach(point,new Point(point->x,point->y),isIgnoreCorner))
                surroundPoints.push_back(new Point(point->x,point->y));
            if( direction.x==0 )
            {
                if(isCanreach(point,new Point(point->x-1,point->y+direction.y),isIgnoreCorner))
                    surroundPoints.push_back(new Point(point->x-1,point->y+direction.y));
                if(isCanreach(point,new Point(point->x+1,point->y+direction.y),isIgnoreCorner))
                    surroundPoints.push_back(new Point(point->x+1,point->y+direction.y));
            }
            else
...
void display( std::vector<std::vector<int> > &_maze , list<myNS::Point *> & _path, myNS::Point &_start,myNS::Point &_end,
              std::list<myNS::Point *> &_openlist, std::list<myNS::Point *> &_closelist)
{
    //  setting
   ...
   //    draw pixel
    ...
    for(auto &p:_path)
    {
       ...
       count++;
     //  cvLine( image, pt3, pt4 , CvScalar(185, 235, 35), 4, 8 , 0);
    ...
    {
           cv::Point pt7 = cvPoint( ( p->x)*pixScale.x+ pixScale.x/2, ( p->y )*pixScale.y + pixScale.y/2);        // x,y iis opposite in cvPoint and vector
       //    cv::Point pt7 = cvPoint( ( p->y)*pixScale.y+ pixScale.y/2, ( p->x )*pixScale.x + pixScale.x/2);        // x,y iis opposite in cvPoint and vector
           cvCircle( image, pt7, 5 , CvScalar(142, 100, 200), -1, 8 , 0);
    }
...
{  
    ...
    //设置起始和结束点  
    myNS::Point start(30,15);
    myNS::Point end(60,30);
    //A*算法找寻路径  

    std::list<myNS::Point *> openlist,closelist;

    chrono::steady_clock::time_point t1 = chrono::steady_clock::now();
    list<myNS::Point *> path=astar.GetPath(start,end,false,openlist,closelist);
...
//    for(auto &p:path)
  //      cout<<'('<<p->x<<','<<p->y<<')'<<endl;
    ...
    display(maze, path, start, end, openlist, closelist);
